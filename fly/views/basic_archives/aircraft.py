from django.contrib.auth.decorators import login_required, permission_required
from django.shortcuts import render, redirect, get_object_or_404
from django import forms

from fly import models as fly_models
from fly.utils import pagination, permission_dict
from fly.utils.bootstrap import BaseModelForm


class AircraftForm(BaseModelForm):
    class Meta:
        model = fly_models.Aircraft
        exclude = ('is_deleted','created_at','updated_at', 'created_by', 'updated_by')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # 设置状态字段为单选按钮
        radio_list = ['device_type', 'status']
        for item in radio_list:
            choices = getattr(fly_models.Aircraft, f'{item}_choices', None)
            if choices is not None:
                self.fields[item].widget = forms.RadioSelect(choices=choices)
            else:
                raise AttributeError(f"{item}_choices does not exist in Aircraft model.")
        self.fields['status'].initial = 1

        # 定义需要过滤的字段和对应模型
        filter_fields = {
            'aircraft_type': fly_models.AircraftType,
            'company': fly_models.Company,
            'operating_base': fly_models.OperatingBase,
        }

        # 循环设置 queryset
        for field_name, model in filter_fields.items():
            if field_name in self.fields:
                self.fields[field_name].queryset = model.objects.filter(is_deleted=False)


@login_required
@permission_required('fly.view_aircraft', raise_exception=True)
def aircraft(request):
    data_dict = {'is_deleted': False}
    key_query = request.GET.get('searchKey', '').strip()
    status_query = request.GET.get('searchStatus', '').strip()
    aircraft_type_query = request.GET.get('searchAircraftType', '').strip()
    if key_query:
        data_dict['name__contains'] = key_query
    if status_query:
        data_dict['status'] = status_query
    if aircraft_type_query:
        data_dict['aircraft_type'] = aircraft_type_query

    # 构建查询集
    queryset = fly_models.Aircraft.objects.filter(**data_dict)
    status_choices = fly_models.Aircraft.status_choices
    aircraft_type_list = fly_models.AircraftType.objects.filter(is_deleted=False)
    # 应用分页
    page_obj = pagination.Pagination(request, queryset)
    permissions = permission_dict.get_model_permission(fly_models.Aircraft)
    context = {
        'aircraft_list': page_obj.page_queryset,
        'page_string': page_obj.page_html(),
        'key_query': key_query,
        'status_query': status_query,
        'status_choices': status_choices,
        'aircraft_type_query': aircraft_type_query,
        'aircraft_type_list': aircraft_type_list,
    }
    context.update(permissions)
    return render(request, 'basic_archives/aircraft.html', context)


@login_required
@permission_required('fly.add_aircraft', raise_exception=True)
def aircraft_add(request):
    if request.method == 'POST':
        form = AircraftForm(request.POST, request.FILES)
        print(form.errors)
        if form.is_valid():
            obj = form.save(commit=False)
            obj.created_by = request.user
            obj.updated_by = request.user
            obj.save()
            return redirect('aircraft')  # 重定向到项目列表页
    else:
        form = AircraftForm()

    date_fields = ['introduction_date']

    context = {
        'form': form,
        'theme': '航空器',
        'back_url': 'aircraft',
        'date_fields': date_fields,
    }

    return render(request, 'base/base_form.html', context)


@login_required
@permission_required('fly.change_aircraft', raise_exception=True)
def aircraft_edit(request, nid):
    row_object = fly_models.Aircraft.objects.filter(id=nid).first()
    if request.method == 'POST':
        form = AircraftForm(request.POST, request.FILES, instance=row_object)
        if form.is_valid():
            obj = form.save(commit=False)
            obj.updated_by = request.user
            obj.save()
            return redirect('aircraft')
    else:
        form = AircraftForm(instance=row_object)

    date_fields = ['introduction_date']

    context = {
        'form': form,
        'theme': '航空器',
        'back_url': 'aircraft',
        'date_fields': date_fields,
    }

    return render(request, 'base/base_form.html', context)


@login_required
@permission_required('fly.delete_aircraft', raise_exception=True)
def aircraft_delete(request, nid):
    # 获取对应ID的对象，如果不存在则返回404
    obj = get_object_or_404(fly_models.Aircraft, id=nid)
    obj.is_deleted = True
    obj.save()
    return redirect('aircraft')
